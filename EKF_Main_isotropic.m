...
%   Area parameters and number of iterations are global variables
    ...
    %   Geolocation process parameters
        ...
        %   EKF (UKF) initialisation:
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            Q_KF=diag([((Q_EKF))^2 ((Q_EKF))^2]);                             %   Process noise matrix: better to be small std for position and power
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            %---------- < Q_KF must be set up appropriately > ------------%
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            R_KF=R_EKF.^2;                                                	%   Specify noise on alpha: enable if wanted  
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for k=1:N_loops_fb                                                             %   Begin main 'for' loop: for all time steps
    ...
    %   Kalman filtering: EKF (UKF)
        ...
        if (((obs_check==1)&&(k==k_obs))||(re_run_bool==1))              	%   If intersections have begun & first time EKF (UKF) is run
            ...
            %%% and UKF
            [x_state(:,k),P_cov(:,:,k),K_EKF_gain(:,k),innovation(k),h(k)]=EKF_form(x_vec_all(1,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,1),x_state_ini,P_cov_ini,F_KF,G_KF,Q_KF,R_KF);
...
%save result.mat
%Note: The graph present in Assignment Report may change if you uncomment save result.mat
% So please run this file without uncommenting result.mat
